...
def _render_form(values: dict[str, str] | None = None, errors: list[str] | None = None) -> str:
    values = values or {}
    errors = errors or []
    ...
    for faction, limit in CONFIG.faction_limits.items():
        ...
        label = f"{faction} ({current}/{limit})"
        faction_options.append(
            f'<option value="{_escape(faction)}" {selected} {disabled}>{_escape(label)}</option>'
        )

    tariff_options = []
    for tariff in CONFIG.tariffs:
        selected = "selected" if values.get("tariff") == tariff else ""
        tariff_options.append(
            f'<option value="{_escape(tariff)}" {selected}>{_escape(tariff)}</option>'
        )

    tariff_message = _escape(CONFIG.tariff_message)
    ...
    <html lang="ru">
      <head>
        ...
        <title>MAD DAY — регистрация без Telegram</title>
        <style>
          :root {{
            color-scheme: light;
          }}
      ...
      <body>
        ...
        <div id="image-modal" class="modal">
          <div class="modal-content">
            <button id="image-modal-close" class="modal-close" type="button">X</button>
